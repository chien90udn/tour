...
var savePositionAllItems = function(menu) {
    ...
    var nested = $( document ). find('#nestable') .nestable('serialize'),
        btn = $('#save-position');
    btn.button('loading');
    $.post(App.baseUrl + "/admin/themes/menus/" + menu + "/item", {_method: 'PUT', data: nested}, function(data) {
        btn.button('reset');
    });
}
...
var updatePositionItems = function(e) {
    ...
    if (window.JSON) {
        ...
        swal(App.language.error, App.language.unknown_error, 'error');
    }
}

var reloadBuilderContent = function(menu) {
    $.get(App.baseUrl + "/admin/themes/menus/" + menu + "/item", function(data) {
        $("#content").html(data);
...
var editItem = function(menu, item) {
    ...
    $.get(App.baseUrl + "/admin/themes/menus/" + menu + "/item/" + item + "/edit", function(html) {
        btn.parent().hide();
        $("#form").html(html).show();
        editor().init();
...
var confirm_to_delete = function (menu, item) {
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        }
    });
    $.post(App.baseUrl + "/admin/themes/menus/" + menu + "/item/" + item, {_method: 'DELETE',dataType: 'JSON'}, function(data) {
...
$(function() {
    ...
    $( document ).on( "click", "#create_new_item", function(event) {
        ...
        $.get(App.baseUrl + "/admin/themes/menus/" + id + "/item/create", function(html) {
            btn.parent().hide();
            $("#form").html(html).show();
            editor().init();
            // trigger form components
